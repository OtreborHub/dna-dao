...
describe("DAO Contract", function () {
    ...
    async function deployDAOFixture() {
        ...
        const token: DNAERC20 = await hre.ethers.deployContract("DNAERC20", ["DnA Token", "DNA", 100, 1])
        const dao: DNADAO = await hre.ethers.deployContract("DNADAO", [token.getAddress(), 1]);

        await token.connect(addr1).buyDNA({value: 10});
        await token.connect(addr2).buyDNA({value: 10});
    ...
    it("Should allow users to buy shares", async function () {
        ...
        await token.connect(addr1).approve(dao.getAddress(), 10);
    ...
    it("Should allow members to propose decisions without transfer", async function () {
        ...
        await dao.connect(addr1).createProposal(title, description, ethers.getAddress("0x0000000000000000000000000000000000000000"), 0);
    ...
    it("Should allow members to propose decisions with transfer", async function () {
        ...
        await dao.connect(addr1).createProposal(title, description, addr2.address, 1);
        const proposalAddr = generateProposalAddress(title + description);
        const proposals = await dao.getProposals();
        expect(proposalAddr).to.equal(proposals[0].proposalAddr);
    ...
    it("Should allow vote delegation and execute proposal with transfer", async function () {
        ...
        await token.connect(addr1).approve(dao.getAddress(), 10);
        await dao.connect(addr1).buyShares(10);
        await token.connect(addr2).approve(dao.getAddress(), 5);
        ...
        expect(await token.balanceOf(addr2.address)).to.equal(6); // 5 initial + 1 from proposal
    });
    ...
    it("Should execute proposals with majority vote", async function () {
        ...
        proposals = await dao.getProposals();
        expect(proposals[0].executed).to.be.true;
        expect(await token.balanceOf(addr2.address)).to.equal(11);
    });
